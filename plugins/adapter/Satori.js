...
Bot.adapter.push(
    new (class SatoriAdapter {
                    ...
                    if (response.status === 403) {
                        if (errorText.includes("login not found")) {
                            Bot.makeLog("error", [
                                "找不到登录会话：",
                                `self_id (${selfId}) `,
                                `当前可用的登录: ${Object.keys(Bot.bots).filter(id => Bot.bots[id].adapter === this).join(', ')}`,
        ...
        async sendMsg(msg, channelId, selfId) {
            ...
            return await this.sendApi("message.create", {
        ...
        async recallMsg(data, message_id) {
            ...
            for (const id of message_id) {
                try {
                    await this.sendApi("message.delete", {
                    ...
                    if (message.body) {
                        ...
                        if (event.type?.startsWith("message-")) {
                            ...
                            if (bot && bot.adapter === this) {
                                ...
                                Bot.makeLog("error", ["无法找到对应的 bot 实例", {
                                    loginId,
